# SIM-MP-003 — Staged Data Exfiltration via DNS Tunneling — XDR + NGFW Stitch

**Scenario ID**: `SIM-MP-003`
...
**MOAT tier**: MOAT
**Source YAML**: [`../../mp-003-data-staged-exfil-dns-tunnel.yml`](../../mp-003-data-staged-exfil-dns-tunnel.yml)
...
|------|-----------|----------|-----------------|
| step-01 | T1005 | EDR | XDR - /etc/passwd read followed by write to /tmp staging directory |
| step-02 | T1074.001 | EDR | XDR - data staging: tar compression of recently-written files in /tmp  |
| step-03 | T1048.003 | ANALYTICS+EDR+NDR | NGFW - DNS anomaly: sustained TXT query burst with high-entropy 60-cha |
| step-04 | T1048 | ANALYTICS+EDR+NDR | NGFW - HTTP POST with file upload to IOC-listed domain |
...
./run.sh --mode single_ttp --ttp T1005
...
{ "provider": "aws", "modules": ["base", "edr", "ndr"], "scenario_id": "SIM-MP-003" }
```
