...
def render_sigma(ttp: dict[str, Any]) -> Optional[str]:
    ...
    abiocs = (ttp.get("detections") or {}).get("abiocs") or []
    if not biocs and not abiocs:
        return None
    ...
    # Sigma supports multi-doc YAML — emit one doc per BIOC, then one per ABIOC
    # (ABIOCs are logic-bearing behavioral detectors; status: experimental).
    docs: list[str] = []
    # Tag each rule with its kind so the rule_id namespace can't collide.
    bioc_and_abioc = [("bioc", b) for b in biocs] + [("abioc", a) for a in abiocs]
    for idx, (kind, b) in enumerate(bioc_and_abioc, start=1):
        name = b.get("name") or f"{kind}-{idx}"
        severity = _SIGMA_SEVERITY.get((b.get("severity") or "").lower(), "medium")
        rule_id = f"{ttp_id.lower()}-{kind}-{idx:02d}"
...
def render_xql(ttp: dict[str, Any]) -> Optional[str]:
    """Plain-text XQL paste-buffer. One section per BIOC + ABIOC + validation query."""
    biocs = (ttp.get("detections") or {}).get("biocs") or []
    abiocs = (ttp.get("detections") or {}).get("abiocs") or []
    xqls = (ttp.get("detections") or {}).get("xql_queries") or []
    if not biocs and not abiocs and not xqls:
        return None
    ...
    for a in abiocs:
        out.append(f"## ABIOC — {a.get('name', '(unnamed)')} (behavioral-ML, causality-anchored)")
        if a.get("severity"):
            out.append(f"# severity: {a['severity']}")
        if a.get("behavioral_profile"):
            out.append(f"# behavioral_profile: {a['behavioral_profile']}")
        if a.get("causality_anchor"):
            out.append(f"# causality_anchor: {a['causality_anchor']}")
        if a.get("description"):
            out.append(f"# {a['description']}")
        out.append("")
        out.append((a.get("logic") or "").rstrip("\n"))
        out.append("")
...
def render_modeling(ttp: dict[str, Any]) -> Optional[str]:
    """Plan 02 — XDM modeling-rule paste-buffer. One section per modeling rule.

    Emits the `[MODEL: dataset=…]` normalization substrate a DC pastes into
    XSIAM *before* the detection fires, so the POV can prove the
    raw-source → modeled-field → detection chain.
    """
    rules = (ttp.get("detections") or {}).get("modeling_rules") or []
    if not rules:
        return None
    ...
    out: list[str] = [
        f"# {ttp_id} — {title}",
        "# XDM modeling-rule paste-buffer (normalization substrate) generated by "
        "detection_scanner/scripts/export_artifacts.py",
        "",
    ]
    for m in rules:
        out.append(f"## MODELING RULE — {m.get('name', '(unnamed)')} (XDM normalization substrate)")
        if m.get("dataset"):
            out.append(f"# raw source dataset: {m['dataset']}")
        if m.get("vendor") or m.get("product"):
            out.append(f"# vendor/product: {m.get('vendor', '?')} / {m.get('product', '?')}")
        xdm_fields = m.get("xdm_fields") or []
        if xdm_fields:
            out.append(f"# XDM fields populated: {', '.join(xdm_fields)}")
        if m.get("description"):
            out.append(f"# {m['description']}")
        out.append("")
        out.append((m.get("logic") or "").rstrip("\n"))
        out.append("")
...
def clean_exports() -> None:
    ...
    if not EXPORTS_DIR.exists():
        return
    for sub in ("sigma", "xql", "modeling", "correlation", "xsoar_playbook"):
        d = EXPORTS_DIR / sub
        if not d.exists():
            continue
        for p in d.glob("*"):
...
def main() -> int:
    ...
    for ttp in iter_active_ttps(args.ttp):
        ...
        modeling = render_modeling(ttp)
        if modeling:
            rel = f"modeling/{ttp_id}.xql"
            _write(EXPORTS_DIR / rel, modeling)
            produced.append(rel)
