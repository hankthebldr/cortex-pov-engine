...
**The Cortex connection is OPT-IN and READ-ONLY** *(the Phase 1 "no connection" rule is superseded; corrected 2026-08-06 — the old wording said alert read-back was "parked", which contradicted the Measurement-loop section below)*. SimCore's job is still to generate signal INTO the environment via agent-based execution, and it **never writes** to Cortex — `CORTEXSIM_XSIAM_ALLOW_WRITE` and `CORTEXSIM_XSIAM_ALLOW_DESTRUCTIVE` stay default-off. When a credential is configured it **does** read out, on three opt-in paths: tenant health/metrics (`/healthcheck`, XQL over `metrics_*`, `core/integrations/xsiam/`); **alert read-back** for auto-validating seeded results into evidence-backed MTTD (`core/connectors/`, the measurement loop below); and **Tier-2 verification** XQL. Nothing is polled without a flag. **`POST /api/connectors/{kind}/preflight`** answers "is my connection working?" *before* the POV instead of mid-run — staged (config → dns_tls → auth → scope → datasets → clock), every stage reported even when an earlier one degraded, and every response carries `queries_issued` so a preflight run against an injected transport cannot be quoted as tenant proof.
...
**The matrix is five targets, Windows included** — `linux/{amd64,arm64}`,
`darwin/{amd64,arm64}`, `windows/amd64` (`.exe`). `agent/executor` stopped being
POSIX-only when the build-tag split landed (`platform_unix.go` / `platform_windows.go`),
and both producers now emit the `.exe`, so `GET /api/agents/binary?os=windows` serves
a real `PE32+` and `?os=windows` returns a PowerShell installer with no preflight
refusal. See the Windows-execution bullet below for what is still unproven (no Windows
host has executed it).

### Rust Submodule Tools (signalbench, ackbarx, xdrtop)
```bash
make rust-dist            # static-musl linux/amd64 binaries -> rust-dist/ (needs docker, not rustup)
cd sources/<tool> && cargo build --release   # host build; needs a Rust toolchain + crates.io
```
`make rust-dist` (and the `rust-builder` stage in `core/Dockerfile`, which bakes the
same three binaries to `/app/rust-dist/` in the image) exists because
`cargo build --release` on a customer jumpbox needs rustup **and** crates.io egress —
the exact default-deny problem the beacon matrix solves for the Go side. `rust-dist/`
is a build artifact — do not commit it. **Caveat: the bytes are baked but not yet
servable** — there is no `/api/tools/binary/{tool}` route, so today the only way off
the image is `docker cp`. Getting a Rust tool onto a jumpbox is NOT yet a one-liner
the way the beacon is. `signalbench`'s plain `cargo build --release` also fails
(rc=101): `src/techniques/software.rs` does `include_bytes!("../../embedded_binaries/
pacemaker_helper")` and that file must be produced by building `helpers/pacemaker`
first — `make rust-dist` does both phases.
...
- **Windows pull is SERVABLE (2026-08-05).** `scripts/build-agent-dist.sh` and the `agent-builder` stage in `core/Dockerfile` both cross-compile `windows/amd64` to `cortexsim-agent-windows-amd64.exe`, so a deployed image serves it (`GET /api/agents/binary?os=windows&arch=amd64` → 200, verified `PE32+ executable (console) x86-64`) and `?os=windows` returns a PowerShell installer with no `WINDOWS_AGENT_UNAVAILABLE` preflight refusal. **The gap was never in the script** — it has carried `windows/amd64` since the build-tag split; it was the `core/Dockerfile` agent-builder loop, which listed only 4 targets, so `make agent-dist` on a dev box emitted 5 while every deployed image shipped 4. CI never noticed because the `agent` job only proves the beacon *compiles* for Windows, never that the image *ships* it. `tests/installer/test_agent_dist_matrix_parity.py` (source parity, fails closed) plus `make check-agent-shelf` (asserts the BUILT IMAGE, no host mount) keep the two matrices from drifting again. **Still unproven: no Windows host has executed the beacon or the PowerShell installer** — serving a correct PE is not the same as `sc.exe` service creation and PS 5.1 execution working on Server 2022.
...
**Two XSIAM code paths, one substrate (2026-08-06).** `core/connectors/` (measurement loop: alert read-back, matcher, MTTD) and `core/integrations/xsiam/` (the ~116 read-only operation packs + Tier-2 XQL) keep their two **call shapes** and their two **credential kinds** deliberately — configuring reconcile must not silently authorise XQL. What they no longer fork is the **substrate**: one XQL result unwrapper (`integrations/xsiam/queries.py::result_rows`/`row_count`), one tenant-URL validator (`tenant_url.py`, enforced at PUT time so a bad URL never reaches a credential), one advanced-auth signer, one UTC timestamp coercer (`connectors/base.py::coerce_utc`), one error vocabulary (`integrations/xsiam/codes.py`) and one per-tenant quota ledger (`ledger.py`). Each has a mechanical AST drift guard asserting exactly one definition. This closed six defects that turned infrastructure problems into claims about the customer's coverage — most sharply: two ad-hoc unwrappers read `reply["results"]` as a list while XSIAM returns `{"data": [...]}`, so a row-count probe measured `1` for a detection that never fired (**PASS**) and a rows probe measured `[]` for a healthy tenant (**FAIL**), and a host-local timestamp coercer shifted every observation by the DC's UTC offset so the matcher silently matched nothing. **An unrecognised envelope now raises rather than returning 0** — the tolerant fix would have kept the bug and greened the tests.

**Preflight before the POV, not during it.** `POST /api/connectors/{kind}/preflight` distinguishes unreachable / unauthorized / authorized-but-not-entitled / working, plus dataset presence (opt-in, priced per query) and **clock skew** (non-optional — MTTD is measured across two clocks). Console destination: **Readiness** under *Manage* (`#/readiness`), which renders the connector ladder as four never-collapsed rungs — **AUTHORED · CONFIGURED · REACHABLE · VERIFIED** — each with what it proves and what it does *not*, and states `tenant-verified: N` verbatim. `GET /api/health` is the one diagnostic surface: **a zero is degraded, not ok** (booting without `tools/` used to report `{status: ok, count: 0}` while every `adapter_ref` in the corpus was unresolvable), it carries `degraded_components` + per-component `code`/`detail`/remediation, and it publishes `not_checked[]` — five reachability-shaped claims it deliberately does *not* make, each naming the endpoint that does. It makes **zero outbound calls**; `integrations` there counts credentials configured, never tenants reached.
...
### The payload shelf — staged, digest-pinned tool artifacts

**The problem it exists for.** Every tier-4 pack installs its tool **from the public internet, on the target host, at dispatch** (`command -v hydra || apt-get install -y hydra`). Customers who buy Cortex run default-deny egress, so that is the first thing their network blocks. A step whose tool never arrived **runs anyway**, produces no detection, and the absent detection reads in a POV report as *"Cortex missed it"* — a manufactured false negative on the customer's stack, in a document a DC shows a customer.

A tier-4 pack may declare **one** staged artifact, `install.artifact` (`filename`/`url`/`kind: file`/`sha256`/`pin`/`license`/`stage_path`/`mode`), validated by seventeen **`TA-01..TA-17`** codes in `adapter_loader.py` (`TA-` and not `A-`, because `assertions.py` already owns `A-10..A-24`). `payloads/sources.json` is **generated** from those declarations (`python3 -m engine.payload_shelf --write|--check`), never hand-maintained. `scripts/build-payloads.sh` stages the bytes onto the DC's own SimCore; `PAYLOAD_ALLOW_UNPINNED=0` refuses an unpinned build. One resolver — `core/engine/payload_shelf.py::compose()` — walks `scenario → adapter_ref → pack → artifact → shelf` and **refuses at compose time** with `PAYLOAD_NOT_STAGED` or `PAYLOAD_PIN_MISMATCH` (409). Served plane-agnostically at `/api/shelf/{payloads,payload/{name},payload/{name}/sha256,artifacts,compose,resolve/{id},stage}`; `/api/k8s/*` stays mounted forever from the **same handler functions** because emitted manifests hard-code those paths in customers' GitOps repos.

**The integrity model — do not fork it.** The digest is recomputed from the shelf bytes on the DC's own SimCore at compose time and baked into whatever the consumer carries. The consumer verifies against a value it **carried in**, never one it fetched from the server it is trusting. The **destination path** — never the shelf key — is overridable (`artifact.stage_path ← external_tools[].stage_as ← request override`), which is what makes a **rename negative control** expressible: stage linpeas as `/tmp/.cache/sysinfo.sh` and the filename-keyed BIOCs correctly go dark while behavioural ones must still fire. Every rename emits `FILENAME_KEYED_DETECTIONS_SUPPRESSED` stating the inverted reading.

**Counted state (verified 2026-08-06 in the prod image).** `packs 91 · rejected 0 · per-tier {1:3, 2:1, 3:20, 4:56, 5:11} · tier4 staged 8 · exempt 48 · undeclared 0 · backlog 8 · settled 40`. The 8 staged are the Linux-privesc-enumerator family (`TOOL-LINPEAS`, `TOOL-PSPY`, `TOOL-SUID3NUM`, `TOOL-LSE`, `TOOL-LINENUM`, `TOOL-DEEPCE`, `TOOL-TRAITOR`, `TOOL-AMICONTAINED`); `unbound[]` is zero. **Count, never quote** — the snippet is in `tools/packs/README.md` § Counting, and a naive `grep -c` over `tools/packs/*.yml` is off by one because `_schema.yml` carries a `tier:` line and an example `artifact_exempt` block. Per-code triage: `docs/reference/adapter-catalog.md` §9.2; where a DC does and does not see it: §9.3.

**Every tier-4 pack now declares WHY it is not shelf-backed.** Since 2026-08-06 a tier-4 pack must carry **exactly one** of `install.artifact` or `install.artifact_exempt {reason_code, reason, revisit?}` — `TA-13` **rejects** a pack declaring neither (a reject, not a warn: a boot warning is how 48 packs came to share one byte-identical non-explanation), `TA-14` rejects both, `TA-15` keeps it tier-4-only, `TA-16` rejects placeholder reasons, `TA-17` requires a `revisit` on the three codes describing a *CortexSim* limitation rather than a property of the tool. Closed vocabulary, measured on the tree: `DISTRO_PACKAGE` 18 · `LANGUAGE_PACKAGE_MANAGER` 12 · `SOURCE_TREE_REQUIRED` 7 · `ARCHIVE_ONLY_NO_EXTRACTOR` 6 · `NEEDS_RUNTIME_DATA` 2 · `LICENCE_NO_REDISTRIBUTION` 1 · `ARCHIVE_MEMBER_NOT_SELF_CONTAINED` 1 · `ARTIFACT_TOO_LARGE` 1. **40 settled · 8 backlog.** `kind: archive` is REJECTED (`TA-08`) and that is the whole remaining opportunity: no consumer can unpack one — the beacon's `Artifact` struct has no `Kind` field, the K8s init container is `wget`+`mv` on an image without `unzip`, and `build-payloads.sh` hard-fails on it. Do not "close the gap" by declaring archives.

**Shelf pull is wired end-to-end** *(the prior "⚠ NOT END-TO-END YET" note here is resolved — `orchestrator.py:639` calls `compose()`, and `payload_shelf.py:462` projects `url`/`dest_path` onto the beacon's `path`/`dest` in one named place)*. Proven live: a real beacon staged linpeas/pspy/suid3num from SimCore, and a tampered shelf file was refused at launch with `PAYLOAD_PIN_MISMATCH` before it reached the target. **Still open:** the exemption `reason_code` does not reach the console — `_adapter_summary()` in `core/api/tools.py` and `UnstagedAdapter` in `payload_shelf.py` both omit it, so `unstaged_adapters[]` still emits one generic sentence for all 48. **Read `docs/reference/payload-shelf.md` §9 + §11 before building on this** — they list every remaining item by owner.
