...
**Counted state (2026-08-05).** 91 adapters, 0 rejected, **8 staged of 56 tier-4 packs — 48 still fetch from the internet on the target**. The 8 are the Linux-privesc-enumerator family (`TOOL-LINPEAS`, `TOOL-PSPY`, `TOOL-SUID3NUM`, `TOOL-LSE`, `TOOL-LINENUM`, `TOOL-DEEPCE`, `TOOL-TRAITOR`, `TOOL-AMICONTAINED`). `unbound[]` is zero. The remaining 48 are apt/pip/git-clone/archive families that are **not shelf-able** without new machinery — see `docs/reference/adapter-catalog.md` §9.2.

**⚠ NOT END-TO-END YET. `Orchestrator._handle_pull` does not call `compose()`**, so `Task.artifacts` is empty on every real pull launch and no beacon stages anything; and `compose()` emits `url`/`dest_path` while the beacon's `ArtifactSpec` requires `path`/`dest`, so the two halves do not join. Both are verified defects, not speculation. **Read `docs/reference/payload-shelf.md` §9 before building on this** — it lists every remaining item by owner.
