...
> **Canonical scenario count.** The counted ground truth (verified 2026-06-10) is **68 loadable scenarios**, all `status: active`, loaded by the schema validator at boot with **0 rejected · 0 dangling `ttp_ref` · 0 dangling `adapter_ref`** — including the 6 AI_SPM scenarios, the CSPM/ASM/TIM scenarios (ASM now 3: exposed-surface + vuln-scan recon + OSINT), EDR-006/007/008, CDR-006, ITDR-006 (AD roasting), and all 7 NDR scenarios. The detection corpus is **72 TTP cards** (`detection_scanner/ttps/*.json`, 566 detection cards in total). **All 430 scenario `detection_id` slugs resolve to a card detection object (GAP-4 closed, 430/430).** `find scenarios -name '*.yml'` returns **99** files; the 31 that are NOT loadable scenarios are supporting YAML the loader skips: `_schema.yml` (1) + AIRS probe packs `scenarios/airs/probes/llm01..llm10/` (19) + browser campaigns `scenarios/browser/campaigns/` (5) + the SIM-MP-00{1,2,3,5} runnable packages `scenarios/multi_plane/packages/` (6). Do not conflate the raw `.yml` file count with the scenario count. Authoritative inventory: `docs/reference/scenario-catalog.md`; counted ground truth in `docs/reference/README.md`.
