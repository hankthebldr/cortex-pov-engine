...
|-------|--------------|--------|
| CDR | Cortex Cloud / Prisma Cloud Compute | 8 scenarios (container enum · cryptominer · container escape · k8s lateral · wildfire trigger · systemd/cron persistence (`SIM-CDR-006`) · cluster/container posture sweep wiring Trivy/Kube-bench/Kubescape/Gitleaks/Cloudsplaining (`SIM-CDR-007`) · IAM-key abuse + S3 exfil (`SIM-CDR-008`)) + IaC module (EKS) |
...
> **Canonical scenario count.** The counted ground truth (verified 2026-06-15) is **75 loadable scenarios**, all `status: active`, loaded by the schema validator at boot with **0 rejected · 0 dangling `ttp_ref` · 0 dangling `adapter_ref`** — including the 6 AI_SPM scenarios, CSPM-001, ASM (now 4: exposed-surface + vuln-scan recon + OSINT + web-app enumeration), EDR-006/007/008/009, CDR-006/007/008, ITDR-006/007/008, TIM-001/002, and all 7 NDR scenarios. The detection corpus is **76 TTP cards** (`detection_scanner/ttps/*.json`, 676 deployable detection objects — BIOC/XQL/correlation/IOC — plus analytics-module references). **All 494 scenario `detection_id` slugs resolve to a card detection object (GAP-4 closed, 494/494).** `find scenarios -name '*.yml'` returns **106** files; the 31 that are NOT loadable scenarios are supporting YAML the loader skips: `_schema.yml` (1) + AIRS probe packs `scenarios/airs/probes/llm01..llm10/` (19) + browser campaigns `scenarios/browser/campaigns/` (5) + the SIM-MP-00{1,2,3,5} runnable packages `scenarios/multi_plane/packages/` (6). Do not conflate the raw `.yml` file count with the scenario count. Authoritative inventory: `docs/reference/scenario-catalog.md`; counted ground truth in `docs/reference/README.md`.
