...
How to add a new EAL Traffic Simulator plugin.

## TL;DR

Drop a Python file in `core/eal_simulator/plugins/` that defines a
class inheriting from `BaseSimulation`. The default plugin registry
imports every file in that directory at startup and registers any
subclasses it finds — no further wiring is required.
...
- **Pydantic params model** keeps the API self-documenting. It is
  published via `GET /api/eal/plugins/{name}` so the UI can render a
  form.
- **ECS event names** use the form `<plugin>_<action>` (e.g.
  `c2_beacon_request`, `dns_tunnel_query`). Outcome is `success` /
  `failure`.
...
- **Blocking syscalls** (raw sockets, OS DNS): wrap in
  `asyncio.to_thread`.
- **Header case**: lowercase telemetry headers when merging with
  per-request overrides — see the `llm_provider_egress` plugin for the
  pattern.

## Testing

Each plugin gets a parametrised dry-run test in
`tests/eal_simulator/test_plugins.py`. Add a row with the plugin name
and a minimal valid params dict — the test verifies the dry-run path
...
fixtures from `conftest.py` to register the plugin under test, and
mock `httpx.AsyncClient` so no real network traffic is generated. See
`test_plugin_llm_provider_egress.py` and `test_plugin_agentic_egress.py`
for examples.
...
3. Add a row to the [[EAL Simulator]] wiki page.
4. If the plugin emits a new EAL signal, add a scenario YAML under
   `scenarios/<plane>/` that exercises it end-to-end.

## See also

- [[EAL Simulator]] — plugin catalog + campaign model
- [[Architecture]] — three-tier design + plugin model
- [[Scenario Authoring]] — wiring a plugin into a scenario
- [`docs/eal-simulator/plugin-development.md`](https://github.com/hankthebldr/cortex-pov-engine/blob/main/docs/eal-simulator/plugin-development.md) — canonical version
