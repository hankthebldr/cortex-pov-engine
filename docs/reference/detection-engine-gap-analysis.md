...
| DL-05 | Single-scenario planes can't sustain a plane-focused POV | detection-library | `scenarios/cspm/` = 1 file (`cspm-001`); TIM = 2; ASM/EMAIL = 4 each | A CSPM- or TIM-centric POV has one demo; if the customer env doesn't reproduce that finding shape there's no fallback. Reads as checkbox coverage. | Set a per-plane floor of 3: CSPM (compliance-drift, IAM exposure, encryption/logging gaps), TIM (STIX lifecycle, feed-to-detection stitch, FP suppression) | M |
